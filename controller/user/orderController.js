...
const downloadInvoice = async (req, res) => {
  try {
    ...
    if (order.address) {
      doc.text(
        `Address: House/Flat ${order.address.flat}, ${order.address.area}, ${order.address.city}`
      );
    ...
    order.orderedItems.forEach((item) => {
      if (item.status !== 'cancelled') {
        ...
        // Price
        doc.text(`₹${item.price.toFixed(2)}`, 320, rowY, {
          width: 80,
          align: 'right',
        });

        // Total
        doc.text(`₹${total.toFixed(2)}`, 420, rowY, {
          width: 80,
          align: 'right',
        });
    ...
    doc.text(`Subtotal: ₹${subtotal.toFixed(2)}`, summaryX, doc.y, {
      width: summaryWidth,
      align: 'right',
    });

    if (order.discount > 0) {
      doc.text(`Discount: -₹${order.discount.toFixed(2)}`, summaryX, doc.y, {
        width: summaryWidth,
        align: 'right',
      });
    }
    ...
    if (order.couponDiscount > 0) {
      doc.text(`Coupon Discount: -₹${order.couponDiscount.toFixed(2)}`, summaryX, doc.y, {
        width: summaryWidth,
        align: 'right',
      });
    }

    doc.text(`Delivery Charge: ₹${(order.deliveryCharge || 0).toFixed(2)}`, summaryX, doc.y, {
      width: summaryWidth,
      align: 'right',
    });
    doc.text(`GST: ₹${(order.gstAmount || 0).toFixed(2)}`, summaryX, doc.y, {
      width: summaryWidth,
      align: 'right',
    });

    doc.font('Helvetica-Bold').text(`Total: ₹${order.finalAmount.toFixed(2)}`, summaryX, doc.y, {
      width: summaryWidth,
      align: 'right',
    });
    doc.moveDown(1);
...
const cancelSingleItem = async (req, res) => {
  try {
    ...
    if (!item || item.status === 'cancelled') {
      return res.status(400).json({
        success: false,
        message: 'item already cancelled or not found',
      });
    }
    ...
    if (order.paymentMethod === 'wallet' || order.paymentMethod === 'online') {
      ...
      const totalBeforeCancellation = order.orderedItems.reduce(
...
const returnOrderItem = async (req, res) => {
  try {
    ...
    if (!item || item.status === 'returned') {
      return res.status(400).json({
        success: false,
        message: 'Item already return requested or invalid',
      });
    }
