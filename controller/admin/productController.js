...
const addProducts = async (req, res) => {
  ...
  const getAllProducts = async (req, res) => {
    try {
      ...
      // By default, only show active products (if your schema uses this)
      // Comment out this section if you don't use an isActive field
      if (req.query.showAll !== 'true') {
        // Uncomment the appropriate filter based on your schema:
  ...
  const addProductOffer = async (req, res) => {
    try {
      ...
      //
      const hasProductOffer = product.productOffer > percentage;
  if (hasProductOffer) {
    return res.json({ success: false, message: 'Product already has a better offer' });
  }
      ...
      console.log('Offer added Successfully',product,'percentage:',percentage)
      return res.json({ success: true, message: "Offer added successfully" });
    } catch (error) {
      console.error("Error in addProductOffer:", error);
      return res.status(500).render('admin/error', {
      pageTitle: 'Error',
      message: 'Something went wrong while loading Add Product page'
  });
  ...
  //   try {
  //     const productId = req.params.id
  //     const { productName, brand, category, regularPrice, salePrice, quantity } = req.body
  //     const updatedProduct = await Category.findByIdAndUpdate(productId, { productName,brand, category, regularPrice, salePrice, quantity }, { new: true })
  //     if (!updatedProduct) {
  //       return res.status(404).json({ success: false, message: "Product not found" })
  //     }
